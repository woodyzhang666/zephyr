...
volatile uint8_t dap_xfer_abort;
    ...
    if ((request_value & DAP_TRANSFER_RnW) != 0U) {
      ...
      if (post_read) {
        ...
        if ((request_value & (DAP_TRANSFER_APnDP | DAP_TRANSFER_MATCH_VALUE)) == DAP_TRANSFER_APnDP) {
          ...
          } while ((response_value == DAP_TRANSFER_WAIT) && retry-- && !dap_xfer_abort);
        } else {
      ...
      if ((request_value & DAP_TRANSFER_MATCH_VALUE) != 0U) {
        ...
        } while (((data & dap_data.transfer.match_mask) != match_value) && match_retry-- && !dap_xfer_abort);
